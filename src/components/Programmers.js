...
export default function Programmers() {
  ...
  const[nameArr] = useState(listOfAwesome);
  const[button,setId] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return nameArr[button-1].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: button ?'gold':'royalblue', // 🤔 color turns to gold, when celebrating
  };
